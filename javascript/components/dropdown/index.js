...
const Dropdown = (props) => {
  ...
  return (
    ...
    >
      <Select
        ...
        filterOption={(input, option) => {
          ...
          if (searchType === 'valueAndText') {
            filterResult = optionText.includes(inputText) || optionValue.includes(inputText);
      ...
      >
        {withNoneOption && <Option
          key={-1}
          value={''}> {'None'}</Option>}
        {withGrouped ?
          Object.keys(groupedOptions).length &&
              Object.keys(groupedOptions).map((group, index) => <OptGroup key={group} label={group}>
                {groupedOptions[group]?.map((item) => <Option key={`${index}_${item.name}`}value={item.id}>
                  {item.id}
                </Option>)}
              </OptGroup>) :
